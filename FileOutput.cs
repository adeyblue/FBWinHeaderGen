...
namespace MetadataParser
{
    ...
    class OutputCreator
    {
        ...
        public void WriteNamespaceHeaders(App.OptionSet options, Dictionary<string, Dictionary<string, string>> injections)
        {
            ...
            Dictionary<string, string> defFileInjections;
            if (injections.TryGetValue("deffiles", out defFileInjections))
            {
                string defFunctions = defFileInjections["functions"];
                if(!String.IsNullOrEmpty(defFunctions))
                {
                    AddFunctionsToDefFiles(defFunctions, defMap);
                }
            }
        ...
        private void AddFunctionsToDefFiles(string functionDefs, DefFileMapping defMap)
        {
            string[] lines = functionDefs.Split('\n', StringSplitOptions.RemoveEmptyEntries);
            foreach(string entry in lines)
            {
                string[] bits = entry.Trim().Split(',', StringSplitOptions.RemoveEmptyEntries);
                if(bits.Length == 4)
                {
                    defMap.AddExport(bits[0], bits[1], Convert.ToInt32(bits[2], 10), Convert.ToInt32(bits[3], 10) != 0);
                }
                else
                {
                    Console.WriteLine("deffile entry '{0}' in fixes file is malformed", entry);
                }
            }
        }
    }
    ...
    class DefFileMapping
    {
        ...
        {
            defFiles = new Dictionary<string, DefFileCreator>(StringComparer.OrdinalIgnoreCase);
        }
    ...
    class FileOutput : IDisposable
    {
        ...
        private void OutputFunction(FunctionType fn)
        {
            ...
            // we don't rename the enum because it's used as the result of several functions and it requires 
